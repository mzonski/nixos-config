# this module is copied from unstable as it would be unwise configure 25.05 module when release is near
{
  lib,
  pkgs,
  config,
...
let
  ...
  cfg = config.services.pocket-id-nixos;

  format = pkgs.formats.keyValue { };
  settingsFile = format.generate "pocket-id-env-vars" cfg.settings;
in
{
  options.services.pocket-id-nixos = {
  ...
  config = mkIf cfg.enable {
    warnings =
      optional (cfg.settings ? MAXMIND_LICENSE_KEY)
        "config.services.pocket-id-nixos.settings.MAXMIND_LICENSE_KEY will be stored as plaintext in the Nix store. Use config.services.pocket-id.environmentFile instead."
      ++
        concatMap
          (
            # Added 2025-05-27
            ...
            optional (cfg.settings ? "${setting}") ''
              config.services.pocket-id-nixos.settings.${setting} is deprecated.
