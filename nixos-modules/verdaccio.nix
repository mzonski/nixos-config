...
let
  ...
  configFile = format.generate configFileName (
    lib.pipe cfg.settings [
      (settings: if settings.listen != null then settings // { inherit listen; } else settings)
      (settings: if !(settings ? storage) then settings // { storage = cfg.dataDir; } else settings)
    ]
  );
in
{
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        assertion = !(cfg.settings ? storage) || cfg.settings.storage == cfg.dataDir;
        message = "services.verdaccio.settings.storage must match services.verdaccio.dataDir or be unset";
      }
    ];
